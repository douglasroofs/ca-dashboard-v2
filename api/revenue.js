...
module.exports = async function handler(req, res) {
      ...
      // JP_LEAP_TOKEN = OAuth access_token from ls.AppUser.token.access_token in LEAP browser session
      // JP_REFRESH_TOKEN = refresh_token for auto-renewal (optional, for future use)
        ...
        if (!resp.ok) {
                  ...
                  return res.status(resp.status).json({
                              ...
                              hint: resp.status === 401
                                ? 'Token expired or invalid. Update JP_LEAP_TOKEN in Vercel env vars with a fresh OAuth access_token from LEAP browser session (ls.AppUser.token.access_token).'
                                            : undefined,
                  });
        }
